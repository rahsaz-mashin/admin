...
const FormFields: FormFieldFunc<T> = (watch, setValue) => {
    ...
    const getPrimaryCurrency = useCallback(async (id: number) => {
        if(!id) setPrimaryCurrency(undefined)
        const data: Currency = await axios.get(`currency/getInfo/${id}`)
        setPrimaryCurrency(data)
    }, [axios])

    const p = watch("primaryCurrency")
    useEffect(() => {
        const v = p
        if (v) getPrimaryCurrency(v as unknown as number).then(r => {})
    }, [p, getPrimaryCurrency]);


    const getSecondaryCurrency = useCallback(async (id: number) => {
        if(!id) setSecondaryCurrency(undefined)
        const data: Currency = await axios.get(`currency/getInfo/${id}`)
        setSecondaryCurrency(data)
    }, [axios])
    const s = watch("secondaryCurrency")
    useEffect(() => {
        const v = s
        if (v) getSecondaryCurrency(v as unknown as number).then(r => {})
    }, [s, getSecondaryCurrency]);
