...
import {Autocomplete, AutocompleteItem, AutocompleteSection, getKeyValue, MenuTriggerAction} from "@heroui/react";
...
export type MinorSelectProps<T extends FieldValues> = {
    name: Path<T> ;
    control: Control<T>;
...
export const MinorSelect = <T extends FieldValues,>(props: MinorSelectProps<T>) => {
    const {
        name,
        control,
    ...
    const onSelectionChange = (key: any) => {
        if (!isSearchable) {
            field.onChange(Array.from(Array.from(key?.values()).filter((v) => (!!v))).join(",") || null)
        } else {
