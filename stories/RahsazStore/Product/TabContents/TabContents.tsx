...
export const ProductTabContents = (
    ...
    const goToItem = (i: number) => {
        router.push(pathname + "#" + items[i].key)
    }


    return (
        <ul className="flex-col items-end select-none p-4 hidden lg:flex">
            {items.map((v, i) => {
                return (
                    <li
                        key={i}
                        onClick={() => goToItem(i)}
                        className="flex items-center cursor-pointer group opacity-50 hover:scale-110 hover:opacity-100 py-1 transition-all duration-200"
                    >
                        <span
